...
def RPanalysis(PcapDirectory, Side, PacketNum, Length, original, analyzerI):
    ...
    while analysis[0][1] - analysis[0][0] > 4:
        ...
        logger.info('Just Checked Regions {} and {}, results are {} and {}, RAque so far is {}'.format(LeftMask, RightMask, LeftClass, RightClass, RAque))
        ...
        if LeftClass == original and RightClass != original:
            ...
            logger.error('Just Checked Regions {} and {}, both regions are classified the same as {}, RAque so far is {}'.format(LeftMask, RightMask, LeftClass, RAque))
            break
...
def main(args):
    ...
    # PcapDirectory.split('_')[0] + 'Random_' + PcapDirectory.split('_')[1]
    if '_' in PcapDirectory:
        Classi_Random = Replay(PcapDirectory.split('_')[0] + 'Random_' + PcapDirectory.split('_')[1], nomodify, analyzerI)
    else:
    ...
    # logger.info('Start reverse engineering the Client side classification contents {}'.format(changeMeta))
    ...
    # #
    # logger.info('Start reverse engineering the Server side classification contents {}'.format(changeMeta))
    ...
    #
    # logger.info('End of Classification Analysis, Client side keywords {} \n Server keywords {}'.format(cKeywords, sKeywords))
