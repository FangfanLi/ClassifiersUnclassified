...
def analyzer(args, resultsFolder, xputBuckets, alpha):
    ...
    # Now we need two finalAnalyzer, one uses the (xput, dur) data from client, the other one uses pcap recorded on the server
    ...
    try:
        ...
        # LOG_ACTION(logger, 'Insertion Server result:' + str(resObjServer.tuplify()))
        db.insertResult(resObjServer)
...
def getHandler(args):
    ...
    if command == 'singleResult':
        ...
        if responseC:
            ...
            return json.dumps({'success': True, 'response': responseS[0]}, cls=myJsonEncoder)
