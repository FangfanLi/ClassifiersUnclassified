...
def finalAnalyzer(userID, historyCount, testID, path, xputBuckets, alpha, side='Server'):
    # Check whether client throughput data is already received, if yes, proceed, if no, return None
    ...
    if side == 'Client':
        ...
        regexTest     = '*_' + str(historyCount) + '_' + str(testID) + '.pickle'
        regexOriginal   = '*_' + str(historyCount) + '_' + str(0) + '.pickle'
        fileTest      = glob.glob(folder+regexTest)
        fileOriginal    = glob.glob(folder+regexOriginal)
        try:
            (xputT, durT) = pickle.load(open(fileTest[0], 'r') )
            (xputO, durO) = pickle.load(open(fileOriginal[0], 'r'))
            xputT = [x for x in xputT if x > 0]
            xputO = [x for x in xputO if x > 0]

        # This means we don't have client side analysis
    ...
    resultFile = (path + '/' + userID + '/decisions/'+'results_{}_{}_{}_{}.pickle').format(userID, side, historyCount, testID)
    forPlot, results        = testIt(xputO, xputT, resultFile, xputBuckets, alpha)
    ...
    plotFile = path + '/' + userID + '/plots/xput_{}_{}_{}_{}_{}_{}_{}_{}.png'.\
        format(userID, side, historyCount, testID, pcapO.replayName, resultObj.area_test, resultObj.ks2pVal, resultObj.ks2_ratio_test)
...
def testIt(xputO, xputT, resultFile, xputBuckets, alpha, doRTT=True):
    forPlot         = {}
